...
async def add(tgClient,users, mychannel):
    ...
    while True:
        ...
        if  not user:
            break
        try:
            await tgClient(InviteToChannelRequest(channel_entity, [user]))
            time.sleep(10)
        except UserPrivacyRestrictedError as userErr:
            print("Privacy error", userErr)
            continue
        except UserNotMutualContactError as err:
            continue
       
        except Exception as err:
            print("An unknown error occur",err)
           
            st = str(err)
            tm = 0
            try:
                tm = int(re.search(r'\d+',st).group())
            except:
                continue
            while tm:
                print("seconds to sleep",tm, end="\r") 
                time.sleep(1)
                tm-=1
